...
export default function Collections({ settings }) {
  ...
  const mostViewed = [...products].sort((a, b) => (b.views || 0) - (a.views || 0)).filter(p => (p.views || 0) > 0).slice(0, 30)
  ...
  const handleReviewSubmit = async () => {
    if (!reviewForm.comment.trim()) { setReviewError('Please leave a comment.'); return }

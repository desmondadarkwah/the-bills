...
const STYLES = `
  ...
  * { box-sizing: border-box; }
  .adm-root { font-family: 'Barlow', sans-serif; min-height: 100vh; background: #f5f0e8; color: #0a0806; }
  .adm-topbar { position: sticky; top: 0; z-index: 50; background: #0a0806; border-bottom: 1px solid rgba(201,147,58,0.15); display: flex; align-items: center; justify-content: space-between; padding: 0 28px; height: 60px; }
  .adm-topbar-left { display: flex; align-items: center; gap: 14px; }
  .adm-logo-badge { width: 36px; height: 36px; background: #c9933a; display: flex; align-items: center; justify-content: center; font-family: 'Cormorant Garamond', serif; font-weight: 700; font-size: 13px; color: #0a0806; letter-spacing: 0.04em; flex-shrink: 0; }
  .adm-logo-name { font-family: 'Cormorant Garamond', serif; font-weight: 600; font-size: 16px; letter-spacing: 0.15em; text-transform: uppercase; color: #f5ede0; line-height: 1; margin-bottom: 2px; }
  ...
  .adm-admin-email { font-size: 12px; color: rgba(255,255,255,0.35); display: none; }
  @media (min-width: 768px) { .adm-admin-email { display: block; } }
  .adm-btn-site { padding: 7px 14px; border: 1px solid rgba(255,255,255,0.15); color: rgba(255,255,255,0.6); background: transparent; cursor: pointer; text-decoration: none; font-family: 'Barlow', sans-serif; font-weight: 500; font-size: 11px; letter-spacing: 0.14em; text-transform: uppercase; transition: border-color 0.2s, color 0.2s; }
  .adm-btn-site:hover { border-color: #c9933a; color: #c9933a; }
  .adm-btn-logout { padding: 7px 16px; background: #c9933a; color: #0a0806; border: none; cursor: pointer; font-family: 'Barlow', sans-serif; font-weight: 600; font-size: 11px; letter-spacing: 0.14em; text-transform: uppercase; transition: opacity 0.2s; }
  .adm-btn-logout:hover { opacity: 0.88; }
  .adm-layout { display: flex; min-height: calc(100vh - 60px); }
  .adm-sidebar { width: 220px; flex-shrink: 0; background: #0a0806; border-right: 1px solid rgba(201,147,58,0.08); padding: 24px 12px; position: sticky; top: 60px; height: calc(100vh - 60px); overflow-y: auto; display: none; }
  @media (min-width: 768px) { .adm-sidebar { display: block; } }
  .adm-sidebar-label { font-family: 'Barlow', sans-serif; font-weight: 500; font-size: 9px; letter-spacing: 0.3em; text-transform: uppercase; color: rgba(255,255,255,0.2); padding: 0 12px; margin-bottom: 8px; margin-top: 16px; }
  .adm-sidebar-label:first-child { margin-top: 0; }
  .adm-nav-btn { width: 100%; text-align: left; display: flex; align-items: center; gap: 10px; padding: 10px 12px; border: none; background: transparent; cursor: pointer; font-family: 'Barlow', sans-serif; font-weight: 500; font-size: 12px; letter-spacing: 0.08em; text-transform: uppercase; color: rgba(255,255,255,0.35); transition: color 0.2s, background 0.2s; position: relative; }
  ...
  .adm-nav-btn:hover { color: rgba(255,255,255,0.7); background: rgba(255,255,255,0.03); }
  .adm-nav-btn.active { color: #c9933a; background: rgba(201,147,58,0.07); }
  .adm-nav-btn.active::before { transform: translateY(-50%) scaleY(1); }
  .adm-nav-icon { font-size: 15px; flex-shrink: 0; }
  .adm-mobile-nav { display: flex; position: fixed; bottom: 0; left: 0; right: 0; z-index: 50; background: #0a0806; border-top: 1px solid rgba(201,147,58,0.1); overflow-x: auto; }
  @media (min-width: 768px) { .adm-mobile-nav { display: none; } }
  .adm-mobile-btn { flex-shrink: 0; display: flex; flex-direction: column; align-items: center; padding: 8px 14px; border: none; background: transparent; cursor: pointer; font-family: 'Barlow', sans-serif; font-weight: 500; font-size: 9px; letter-spacing: 0.1em; text-transform: uppercase; color: rgba(255,255,255,0.3); transition: color 0.2s; gap: 3px; }
  ...
  .adm-page-title { font-family: 'Cormorant Garamond', serif; font-weight: 600; font-size: 32px; text-transform: uppercase; letter-spacing: 0.06em; color: #0a0806; margin: 0 0 28px; display: flex; align-items: center; gap: 12px; }
  .adm-page-title span { font-size: 24px; }
  .adm-card { background: #fff; border: 1px solid rgba(10,8,6,0.06); overflow: hidden; margin-bottom: 20px; }
  .adm-card-head { padding: 16px 20px; border-bottom: 1px solid rgba(10,8,6,0.05); display: flex; align-items: center; justify-content: space-between; }
  .adm-card-title { font-family: 'Cormorant Garamond', serif; font-weight: 600; font-size: 16px; letter-spacing: 0.1em; text-transform: uppercase; color: #0a0806; }
  .adm-row { padding: 14px 20px; border-bottom: 1px solid rgba(10,8,6,0.04); display: flex; align-items: center; gap: 14px; transition: background 0.15s; }
  .adm-row:last-child { border-bottom: none; }
  .adm-row:hover { background: rgba(10,8,6,0.02); }
  .adm-row-name { font-weight: 600; font-size: 13px; color: #0a0806; margin-bottom: 2px; }
  .adm-row-sub { font-size: 11px; color: rgba(10,8,6,0.4); }
  .adm-section-head { display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px; }
  .adm-btn { padding: 8px 18px; border: none; cursor: pointer; font-family: 'Barlow', sans-serif; font-weight: 600; font-size: 11px; letter-spacing: 0.16em; text-transform: uppercase; transition: opacity 0.2s; display: inline-flex; align-items: center; gap: 6px; }
  .adm-btn:hover { opacity: 0.85; }
  .adm-btn:disabled { opacity: 0.45; cursor: not-allowed; }
  .adm-btn-gold { background: #c9933a; color: #0a0806; }
  .adm-btn-dark { background: #0a0806; color: #fff; }
  .adm-btn-red { background: #dc2626; color: #fff; }
  .adm-btn-ghost { background: transparent; border: 1px solid rgba(10,8,6,0.2); color: #0a0806; }
  .adm-btn-sm { padding: 5px 12px; font-size: 10px; }
  .adm-badge { display: inline-block; padding: 3px 10px; font-family: 'Barlow', sans-serif; font-weight: 600; font-size: 10px; letter-spacing: 0.12em; text-transform: uppercase; }
  .adm-badge-gold { background: rgba(201,147,58,0.12); color: #8a5e1a; border: 1px solid rgba(201,147,58,0.3); }
  .adm-badge-green { background: rgba(5,150,105,0.08); color: #065f46; border: 1px solid rgba(5,150,105,0.2); }
  .adm-badge-red { background: rgba(220,38,38,0.08); color: #991b1b; border: 1px solid rgba(220,38,38,0.2); }
  .adm-item-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(240px, 1fr)); gap: 12px; }
  .adm-item-card { background: #fff; border: 1px solid rgba(10,8,6,0.06); overflow: hidden; position: relative; transition: box-shadow 0.2s; }
  .adm-item-card:hover { box-shadow: 0 6px 24px rgba(10,8,6,0.1); }
  .adm-item-img { width: 100%; height: 200px; object-fit: cover; display: block; }
  .adm-item-placeholder { width: 100%; height: 200px; background: #0a0806; display: flex; align-items: center; justify-content: center; font-family: 'Cormorant Garamond', serif; font-weight: 700; font-size: 48px; color: rgba(201,147,58,0.15); letter-spacing: 0.1em; }
  .adm-item-body { padding: 14px 16px; }
  .adm-item-name { font-family: 'Cormorant Garamond', serif; font-weight: 600; font-size: 16px; text-transform: uppercase; letter-spacing: 0.06em; color: #0a0806; margin-bottom: 4px; }
  .adm-item-desc { font-size: 11px; color: #888; line-height: 1.5; margin-bottom: 10px; }
  .adm-item-actions { display: flex; gap: 6px; }
  .adm-modal-backdrop { position: fixed; inset: 0; z-index: 100; background: rgba(10,8,6,0.7); backdrop-filter: blur(4px); display: flex; align-items: center; justify-content: center; padding: 16px; }
  .adm-modal { background: #fff; width: 100%; max-width: 560px; max-height: 90vh; overflow-y: auto; }
  .adm-modal-head { padding: 20px 24px; border-bottom: 1px solid rgba(10,8,6,0.06); display: flex; align-items: center; justify-content: space-between; position: sticky; top: 0; background: #fff; z-index: 1; }
  .adm-modal-title { font-family: 'Cormorant Garamond', serif; font-weight: 600; font-size: 20px; text-transform: uppercase; letter-spacing: 0.08em; color: #0a0806; }
  .adm-modal-close { background: none; border: 1px solid rgba(10,8,6,0.12); color: rgba(10,8,6,0.4); cursor: pointer; width: 30px; height: 30px; display: flex; align-items: center; justify-content: center; font-size: 16px; transition: all 0.2s; }
  ...
  .adm-label { font-family: 'Barlow', sans-serif; font-weight: 500; font-size: 10px; letter-spacing: 0.2em; text-transform: uppercase; color: rgba(10,8,6,0.45); }
  .adm-input { background: #f8f5f0; border: 1px solid rgba(10,8,6,0.1); color: #0a0806; font-family: 'Barlow', sans-serif; font-size: 13px; padding: 11px 14px; outline: none; width: 100%; transition: border-color 0.2s; }
  .adm-input:focus { border-color: #c9933a; }
  .adm-input::placeholder { color: rgba(10,8,6,0.2); }
  .adm-two-col { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; }
  .adm-check-label { display: flex; align-items: center; gap: 10px; cursor: pointer; font-size: 13px; color: rgba(10,8,6,0.6); font-weight: 500; }
  .adm-status { padding: 12px 16px; font-size: 12px; font-weight: 600; display: flex; align-items: center; gap: 8px; }
  .adm-status.ok { background: rgba(5,150,105,0.07); border-left: 3px solid #059669; color: #065f46; }
  .adm-status.err { background: rgba(220,38,38,0.07); border-left: 3px solid #dc2626; color: #991b1b; }
  .adm-settings-section { background: #fff; border: 1px solid rgba(10,8,6,0.06); margin-bottom: 16px; }
  .adm-settings-head { padding: 16px 20px; border-bottom: 1px solid rgba(10,8,6,0.05); }
  .adm-settings-title { font-family: 'Cormorant Garamond', serif; font-weight: 600; font-size: 16px; letter-spacing: 0.1em; text-transform: uppercase; color: #0a0806; }
  .adm-settings-sub { font-size: 11px; color: rgba(10,8,6,0.35); margin-top: 2px; }
  .adm-settings-body { padding: 20px; }
  .adm-empty { padding: 48px 24px; text-align: center; color: rgba(10,8,6,0.3); }
  .adm-empty-icon { font-family: 'Cormorant Garamond', serif; font-weight: 700; font-size: 56px; color: rgba(10,8,6,0.05); text-transform: uppercase; letter-spacing: -0.02em; margin-bottom: 12px; }
  ...
  .adm-stat-card { background: #fff; border: 1px solid rgba(10,8,6,0.06); padding: 20px; position: relative; overflow: hidden; }
  ...
  .adm-stat-label { font-size: 10px; color: rgba(10,8,6,0.4); letter-spacing: 0.15em; text-transform: uppercase; font-weight: 500; }
`
      ...
      <div className="adm-stat-grid">
        {stats.map(s => (
          <div key={s.label} className="adm-stat-card">
            <div style={{ position:'absolute', top:14, right:14, fontSize:20, opacity:0.15 }}>{s.icon}</div>
        ...
        {products.length === 0
          ...
          : products.slice(0,6).map(p => (
            <div key={p._id} className="adm-row">
              {p.images?.[0]
                ? <img src={p.images[0]} alt={p.name} style={{ width:44, height:44, objectFit:'cover', flexShrink:0 }} />
                : <div style={{ width:44, height:44, background:'#0a0806', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, fontSize:18, color:'rgba(201,147,58,0.3)' }}>TB</div>
              }
              <div style={{ flex:1, minWidth:0 }}>
                <div className="adm-row-name">{p.name}</div>
                <div className="adm-row-sub">{p.category} · {p.price}</div>
              </div>
...
function ProductsTab() {
  ...
  const [form, setForm]         = useState({ name:'', description:'', price:'', category:'', available:true, featured:false, order:0 })
  const [images, setImages]     = useState([])
  const [saving, setSaving]     = useState(false)

  useEffect(() => { load() }, [])
  const load = () => fetchAllProducts().then(d => { setProducts(d); setLoading(false) }).catch(console.error)
  const openAdd  = () => { setEditing(null); setForm({ name:'',description:'',price:'',category:'',available:true,featured:false,order:0 }); setImages([]); setModal(true) }
  const openEdit = p => { setEditing(p); setForm({ name:p.name,description:p.description,price:p.price,category:p.category,available:p.available,featured:p.featured,order:p.order||0 }); setImages([]); setModal(true) }
  ...
  const toggle = async p => {
    ...
    fd.append('price',p.price); fd.append('category',p.category)
            ...
            : products.map(p => (
              <div key={p._id} className="adm-row">
                {p.images?.[0]
                  ...
                  : <div style={{ width:52,height:52,background:'#0a0806',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,fontSize:20,color:'rgba(201,147,58,0.2)',fontFamily:"'Cormorant Garamond',serif" }}>TB</div>
                }
                <div style={{ flex:1, minWidth:0 }}>
                  <div className="adm-row-name">{p.name}</div>
                  <div className="adm-row-sub">{p.category} · {p.price}</div>
...
function CollectionsTab() {
  ...
  const openAdd  = () => { setEditing(null); setForm({ name:'',description:'',active:true,order:0 }); setImage(null); setModal(true) }
  const openEdit = c => { setEditing(c); setForm({ name:c.name,description:c.description,active:c.active,order:c.order||0 }); setImage(null); setModal(true) }
    ...
    <div>
          ...
          : collections.length === 0
            ...
            : collections.map(c => (
              <div key={c._id} className="adm-row">
                {c.image
                  ? <img src={c.image} alt={c.name} style={{ width:52,height:52,objectFit:'cover',flexShrink:0 }} />
                  : <div style={{ width:52,height:52,background:'#0a0806',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,fontSize:18,color:'rgba(201,147,58,0.2)',fontFamily:"'Cormorant Garamond',serif" }}>TB</div>
                }
                <div style={{ flex:1, minWidth:0 }}>
      ...
      {modal && (
        <Modal title={editing ? 'Edit Collection' : 'Add Collection'} onClose={() => setModal(false)}>
          <Field label="Collection Name"><input className="adm-input" value={form.name} onChange={e => setForm({...form,name:e.target.value})} placeholder="e.g. Summer 2024" /></Field>
          <Field label="Description"><textarea className="adm-input" rows={3} value={form.description} onChange={e => setForm({...form,description:e.target.value})} /></Field>
          <Field label="Cover Image"><input type="file" accept="image/*" className="adm-input" onChange={e => setImage(e.target.files[0])} />{editing?.image && !image && <img src={editing.image} alt="" style={{ height:60, marginTop:8, objectFit:'cover' }} />}</Field>
          <div className="adm-two-col">
            <Field label="Display Order"><input type="number" className="adm-input" value={form.order} onChange={e => setForm({...form,order:e.target.value})} /></Field>
      ...
      <div className="adm-card">
        ...
        {admins.map(a => (
          <div key={a._id} className="adm-row">
            <div style={{ width:36,height:36,background:'#0a0806',color:'#c9933a',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:"'Cormorant Garamond',serif",fontWeight:700,fontSize:16,flexShrink:0 }}>{a.email?.charAt(0).toUpperCase()}</div>
            <div style={{ flex:1 }}>
